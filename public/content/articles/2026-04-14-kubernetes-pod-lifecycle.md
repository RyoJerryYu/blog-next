...
| 步骤     | 事件           | Pod Phase | 容器 State  | 容器 ready | Pod ready | 说明                                                                                              |
| ------ | ------------ | --------- | --------- | -------- | --------- | ----------------------------------------------------------------------------------------------- |
| **T0** | 创建 Pod 资源    | `Pending` | `Waiting` | `false`  | `false`   | kubectl 或 Controller 向 API Server 创建 Pod                                                        |
| **T1** | Scheduler 调度 | `Pending` | `Waiting` | `false`  | `false`   | Scheduler 计算节点，写入 `spec.nodeName`                                                               |
| **T2** | Kubelet 创建容器 | `Pending` | `Waiting` | `false`  | `false`   | Kubelet 调用 CRI Runtime ，开始拉取镜像，创建容器。同时 Kubelet 调用 CNI 插件配置网络。                                   |
| **T3** | 容器启动         | `Running` | `Running` | `false`  | `false`   | 容器创建完成，主容器进程启动，Kubelet 更新 `status.podIP` 。但仍未通过就绪探针检查                                           |
| **T4** | 容器 Ready     | `Running` | `Running` | `true`   | `false`   | 容器就绪探针连续成功（≥ `successThreshold`），容器 `ready: true`。但 Pod 可能还未就绪（如未通过 `ReadinessGates`）           |
| **T5** | Pod Ready    | `Running` | `Running` | `true`   | `true` ✅  | 所有条件满足（`ContainersReady=true` + `ReadinessGates` 等），Kubelet 设置 `Ready: true`。Pod 可接收 Service 流量 |

#### 删除流程

| 步骤     | 事件                 | Pod Phase          | 容器 State     | 容器 ready | Pod ready  | 说明                                                                                                                                                            |
| ------ | ------------------ | ------------------ | ------------ | -------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **T0** | 发送删除请求             | `Running`          | `Running`    | `true`   | `true`     | API Server 设置 `deletionTimestamp`                                                                                                                             |
| **T1** | preStop 执行         | `Running`          | `Running`    | `false`  | `false` ⚠️ | Kubelet 开始计时 `terminationGracePeriodSeconds` ，停止执行 `readinessProbe` ，设置容器 `ready: false` → `Pod Ready: false`。preStop hook 开始执行                               |
| **T2** | 从 EndpointSlice 移除 | `Running`          | `Running`    | `false`  | `false`    | EndpointSlice Controller 监听到 `Pod Ready: false`，将 Pod 从 Service 后端移除，停止接收新流量（**K8s 1.22+** 中，这个过程可以提前到 EndpointSlice Controller 监听到 `deletionTimestamp` 被设置时） |
| **T3** | 发送 SIGTERM         | `Running`          | `Running`    | `false`  | `false`    | 向容器主进程发送终止信号（无 preStop hook 或 preStop hook 结束后）。                                                                                                              |
| **T4** | 容器退出               | `Running`          | `Terminated` | `false`  | `false`    | 容器进程退出 ⚠️ Phase 仍为 Running（所有容器都未退出前，或仍在重启策略影响下）                                                                                                              |
| **T5** | Pod Phase 变化       | `Succeeded/Failed` | `Terminated` | `false`  | `false`    | 所有容器终止 + 不再重启 → Kubelet 计算最终 Phase                                                                                                                            |
| **T6** | Pod 删除             | （资源消失）             | （资源消失）       | （资源消失）   | （资源消失）     | Pod 资源从 API Server 删除                                                                                                                                         |
...
> [!info]
> Scheduler 是"一锤子买卖"——它只负责把 Pod 分配到节点，之后的容器启动、探针检查、状态上报等，全部由 Kubelet 接管。这也是为什么 Pod 调度成功（Phase 变为 Running）后，仍可能因为镜像拉取失败、探针失败等原因无法真正提供服务。
